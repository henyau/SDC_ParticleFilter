...
inline void homotransform(double tx, double ty, double theta, double x_in, double y_in, double &x_out, double &y_out) {
	double st = sin(theta);
	double ct = cos(theta);
	x_out = x_in*ct - y_in*st+tx;
	y_out = x_in*st + y_in*ct + ty;
}
